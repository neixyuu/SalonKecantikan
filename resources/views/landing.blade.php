...
@section('content')
    ...
    <section class="relative overflow-hidden"
        style="min-height: 520px; background: linear-gradient(135deg, var(--color-cream) 0%, var(--color-cream-dark) 100%);">
        {{-- Background decorative with image --}}
        <div class="absolute inset-0 flex">
            <div class="w-full md:w-1/2"></div>
            <div class="hidden md:block w-1/2 relative bg-cover bg-center"
                style="background-image: url('https://images.unsplash.com/photo-1600334129128-685c5582fd35?auto=format&fit=crop&q=80&w=1200');">
        ...
        <div class="relative max-w-6xl mx-auto px-6 py-20 md:py-32 flex items-center">
            <div class="max-w-xl fade-in">
                ...
                <p class="text-sm text-charcoal-light leading-relaxed mb-10 max-w-sm">
                    Salon kecantikan premium dengan sentuhan personal. Dari facial mewah hingga perawatan tubuh — kami hadir
                    untuk merawat kecantikan terbaik Anda.
                </p>
        ...
        <div class="absolute top-10 right-10 w-64 h-64 rounded-full opacity-10"
            style="background: radial-gradient(circle, var(--color-tan) 0%, transparent 70%);"></div>
        <div class="absolute bottom-0 right-1/4 w-40 h-40 rounded-full opacity-15"
            style="background: radial-gradient(circle, var(--color-blush) 0%, transparent 70%);"></div>
    </section>

    {{-- ── Promo Banners ── --}}
    <section class="max-w-6xl mx-auto px-6 py-8">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="relative overflow-hidden p-8 flex flex-col justify-center bg-cover bg-center"
                style="min-height: 220px; background-image: linear-gradient(135deg, rgba(243, 217, 212, 0.85) 0%, rgba(232, 196, 188, 0.9) 100%), url('https://images.unsplash.com/photo-1616394584738-fc6e612e71b9?auto=format&fit=crop&q=80&w=800');">
                ...
                <a href="{{ auth()->check() ? '/treatments' : '/register' }}"
                    class="btn-nude inline-flex self-start mt-2 text-xs relative z-10">Reservasi →</a>
            </div>
            <div class="relative overflow-hidden p-8 flex flex-col justify-center bg-cover bg-center"
                style="min-height: 220px; background-image: linear-gradient(135deg, rgba(232, 232, 232, 0.85) 0%, rgba(217, 217, 217, 0.9) 100%), url('https://images.unsplash.com/photo-1544161515-4ab6ce6db874?auto=format&fit=crop&q=80&w=800');">
                ...
                @foreach($treatments as $treatment)
                    <div class="treatment-card fade-in">
                        ...
                        <div class="overflow-hidden" style="background-color: var(--color-graylight);">
                            @if($treatment->image)
                                <img src="{{ $treatment->image_url }}" alt="{{ $treatment->name }}" class="w-full"
                        ...
                        <div class="p-6 text-center">
                            ...
                            <div class="flex justify-center gap-4 mb-4">
                                ...
                                <span class="text-xs font-semibold"
                                    style="color: var(--color-tan-dark);">{{ $treatment->formatted_price }}</span>
                            </div>
    ...
    <section class="py-8" id="tentang">
        <div class="max-w-6xl mx-auto px-6 mb-6">
            <p class="text-center text-xs tracking-widest uppercase text-charcoal-light">♦ Join the AETH Clinic Community
            </p>
        </div>
        <div class="flex gap-1 overflow-x-auto pb-4 px-6 max-w-6xl mx-auto snap-x">
            @php
                $galleryImages = [
            ...
            @endphp
            @for($i = 0; $i < 6; $i++)
                <div class="flex-1 min-w-[140px] md:min-w-0 relative group overflow-hidden snap-center" style="height: 140px;">
                    <div class="absolute inset-0 bg-cover bg-center transition-transform duration-700 group-hover:scale-110"
                        style="background-image: url('{{ $galleryImages[$i] }}');"></div>
                    <div class="absolute inset-0 bg-charcoal/40 group-hover:bg-charcoal/20 transition-colors duration-300">
                    </div>
                    <div class="absolute inset-0 flex items-center justify-center">
                        <span
                            class="text-xs tracking-wider uppercase text-white font-semibold shadow-sm">{{ $galleryLabels[$i] }}</span>
                    </div>
                </div>
            @endfor
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                @foreach($announcements as $announcement)
                    <div class="p-6 border border-graymedium" style="background-color: var(--color-white);">
                        <p class="text-xs text-charcoal-light tracking-widest uppercase mb-2">
    ...
    <section class="py-20 relative overflow-hidden" style="background-color: var(--color-charcoal);">
        <div class="absolute inset-0 bg-cover bg-center opacity-20"
            style="background-image: url('https://images.unsplash.com/photo-1570172619644-dfd03ed5d881?auto=format&fit=crop&q=80&w=1600');">
        </div>
        <div class="max-w-2xl mx-auto px-6 text-center relative z-10">
            ...
            <p class="text-sm text-cream/60 mb-8 tracking-wide">Daftarkan akun Anda dan mulai reservasi layanan salon
                premium kami.</p>
